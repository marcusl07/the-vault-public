...
class SyncPublicMirrorTests(unittest.TestCase):
    def test_sync_tree_copies_publishable_files_and_removes_stale_dest_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            (source_root / "sources" / "chat").mkdir(parents=True)
            (source_root / "sources" / "chat" / "private.md").write_text("private chat\n", encoding="utf-8")
            (source_root / "capture").mkdir()
            (source_root / "capture" / "pipeline.lock").write_text("{}\n", encoding="utf-8")
            (source_root / "reminders.txt").write_text("private reminder\n", encoding="utf-8")
            (source_root / "vault_scaling_handoff.docx").write_bytes(b"private docx")
            ...
            self.assertFalse((dest_root / "sources" / "chat" / "private.md").exists())
            self.assertFalse((dest_root / "capture" / "pipeline.lock").exists())
            self.assertFalse((dest_root / "reminders.txt").exists())
            self.assertFalse((dest_root / "vault_scaling_handoff.docx").exists())
