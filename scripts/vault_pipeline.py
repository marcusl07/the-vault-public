...
def _upsert_wiki_pages_for_note(
    *,
    ...
    effective_budget = budget or _default_maintenance_budget()
    ...
    if router_decision.action == "heavy_update" and not simple_new_page_flow:
        proposal = _build_heavy_update_proposal(
            title=title,
        ...
        _touched_pages, changed_slugs, review_queued = _apply_heavy_update_proposal(
            title=title,
            source_record=source_record,
            loaded_pages=loaded_pages,
            resolved_assignments=resolved_assignments,
        ...
        _append_wiki_ingest_log(title, router_decision=router_decision, deferred_items=proposal.deferred_items)
